...
class CargarLocal(ParamContexto: Context) {
    ...
    fun guardarEjemplos():Boolean{
        try {
            val _listaEjemplos = listOf<Local>(
                Local(0,
                    "La Luna",
                    "Restaurant",
                    "Av. Fontana 656",
            ...
            val CLocal = ControladorLocal(contexto)
            //truncado de la tabla
            CLocal.truncarLocales()
            //cargado de los ejemplos
            for(_local in _listaEjemplos){
                CLocal.agregarLocal(_local)
            }
            return true
